...
- `ui/VoxelHighlight.js` — voxel selection highlight + deletion (click to remove voxels)
...
- `voxels/` — per-voxel definition files (`StoneVoxel.js`, `DirtVoxel.js`, `BasicVoxel.js`, etc.) that declare face-specific texture mappings; consumed by `VoxelTextures.js`
